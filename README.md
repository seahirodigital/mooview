...
2026年6月11日時点のV0.2では、比較チャートの視認性を強化しました。

- 比較銘柄のチャート色を拡張し、多数の銘柄を重ねても色が繰り返されにくくしました。
- 比較ラインへマウスオーバーしたとき、銘柄コードや `CRM/SPY` のような演算コードをチャート上にオーバーレイ表示します。
- 複数比較時、右軸の数値列にチャート色と同じ小さな変動率タブを表示します。タブは軸に密着し、銘柄名は入れず、変動率だけを1行で表示します。
- 変動率タブが縦方向に重なる場合は、順位が読み取れるように上下へずらして表示します。
- RSI/MACDなどのサブチャート右上に、表示と縮小を切り替えるボタンを追加しました。
- パッケージバージョンを `0.2.0` に更新しました。
...
ウォッチリストでは、CtrlクリックまたはShiftクリックで複数銘柄を選択できます。複数選択した状態でチャート右上の `+` を押すと、選択中の銘柄をまとめて比較ラインへ追加します。同じく、複数選択した銘柄をドラッグしてチャートへ投げ込むと、選択中の銘柄をまとめて比較ラインへ追加します。
...
- SMA、EMA、ボリンジャーバンド、RSI、MACD、VRVPなどの指標: 演算式では既定で非アクティブ
