...
左メニューの「マクロ資金フロー」の直下にある「企業開示DB」では、EDINET、EDINET DB、TDNET公式ページ、TDNET WEB APIから取得した企業開示をSQLiteへ蓄積し、PDF本体を常時保存せず一覧表示します。APIキー未登録の配信元があっても画面とDBは正常起動します。
...
- コード列の右に経由列を置き、TDNETは `T`、EDINET・EDINET DBは `E` と表示します。日時は `260805 14:00` 形式です。
- 資料URL、EDINET DB、バフェット・コード列は、横幅を抑えるためリンクアイコンだけを表示します。バフェット・コードは `https://www.buffett-code.com/company/7203/` のように証券コードから生成します。
...
- Gemini要約は画面内の大型ポップアップへ全文表示し、本文領域だけを縦スクロールできます。「全文コピー」で要約全体をクリップボードへコピーでき、閉じるボタン、Escキー、背景クリックで閉じられます。
- TDNET公式ページを既定1分間隔で先にスクレイピングし、TDNET WEB APIを補完経路として後から同期します。両経路のPDFファイル名を共通キーにして同じ開示へ統合し、APIが後着しても二重登録しません。手動同期は左から「TD Sync」「TD API」「ED API」の順に配置します。
- 「全タグ」の左にある「フィルタ」を開くと、TDNET、EDINET、ノイズ除去、大企業のみを個別に切り替えられます。ノイズ除去はマウスオーバーで対象語を確認でき、右クリックでキーワードを編集できます。
- TDNETの証券コードは配信値末尾の不要な0を除いた4桁で扱います。TDNET、EDINET、ノイズ除去、大企業のみを個別に絞り込めます。
