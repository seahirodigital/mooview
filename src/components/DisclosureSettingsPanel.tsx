    ...
    <div data-disclosure-settings="true" className="min-h-0 flex-1 space-y-2 overflow-y-auto p-2 text-xs">
      <SettingsSection title="企業開示DB" icon={<Database className="h-4 w-4 text-emerald-300" />}>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          <button type="button" onClick={() => void runSync('tdnet-scrape')} disabled={syncingSource !== null} className="flex h-8 items-center justify-center gap-1 border border-cyan-900 bg-cyan-950/35 text-[10px] font-bold text-cyan-200 disabled:opacity-45">{syncingSource === 'tdnet-scrape' ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}{syncingSource === 'tdnet-scrape' ? '取得中…' : 'TD Sync'}</button>
          <button type="button" onClick={() => void runSync('tdnet')} disabled={syncingSource !== null} className="flex h-8 items-center justify-center gap-1 border border-blue-900 bg-blue-950/35 text-[10px] font-bold text-blue-200 disabled:opacity-45">{syncingSource === 'tdnet' ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}{syncingSource === 'tdnet' ? '取得中…' : 'TD API'}</button>
